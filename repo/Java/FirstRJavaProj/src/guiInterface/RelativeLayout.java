...
public class RelativeLayout implements LayoutManager2, java.io.Serializable {
     ...
     *
     * For X-AXIS this would refer to the Y alignemt. For Y-AXIS this would
     * refer to the X alignment.
     *
     * Must be between 0.0 and 1.0, or -1. Values can be specified using:
     *
     * RelativeLayout.LEADING RelativeLayout.CENTER RelativeLayout.TRAILING
     * RelativeLayout.COMPONENT - the getAlignemntX/Y method for the opposite
     * axis will be used
     */
    ...
    private int[] allocateRelativeSpace(Container parent, int spaceAvailable, float relativeTotal) {
        ...
        for (int i = 0; i < components; i++) {
            ...
            if (relativeTotal > 0 && spaceAvailable > 0) {
                ...
                if (constraint != null) {
                    int space = (Math.round(spaceAvailable * constraint.floatValue() / relativeTotal));
    ...
    /*
     * Because of rounding, all the space has not been allocated Override this
     * method to create a custom rounding policy
     */
    ...
    /*
     * Largest component using relative sizing gets all the space. When multiple
     * components are the same size, the last one found is used.
     */
    ...
    /*
     * Each component using relative sizing gets 1 more pixel until all the
     * space is used, starting with the first.
     */
