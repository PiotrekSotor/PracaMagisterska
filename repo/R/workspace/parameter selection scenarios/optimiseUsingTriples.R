select123parameters <- function(fun, lower_bounds, upper_bounds, 
  ...
  result_pack <- array(dimnames = list(pMutation=par_1_set, pCrossover=par_2_set, popSize=par_3_set),
                      dim = c(length(par_1_set),length(par_2_set),length(par_3_set)))
  ...
  for (par_1 in par_1_set){
    for (par_2 in par_2_set){
      for(par_3 in par_3_set){
        ...
        result_pack[toString(par_1), toString(par_2), toString(par_3)] = best_solution/num_of_tests
      }
    }
  }
  stop_time <- proc.time() - start_time
  
  best_par_3_fitness = min(result_pack[,,1])
  best_par_3_value = par_3_set[1]
  for (par_3 in par_3_set){
    if (min(result_pack[,,toString(par_3)]) < best_par_3_fitness){
      best_par_3_value = par_3   
    }
  }
  par_1_par_2_indexes <- getIndexesOfOptim(result_pack[,,toString(best_par_3_value)], maxSearch = FALSE)
  selected_params_values <- c(par_1_set[par_1_par_2_indexes[1]], par_2_set[par_1_par_2_indexes[2]], best_par_3_value)
  
  
  return (list(time=stop_time, params=selected_params_values, best_fitness = min(result_pack)))
}
...
select456parameters <- function(fun, lower_bounds, upper_bounds, 
  ...
  result_pack <- array(dimnames = list(poptim=par_4_set, pressel=par_5_set, method=par_6_set),
                       dim = c(length(par_4_set),length(par_5_set),length(par_6_set)))
  start_time <- proc.time()  
  for (par_4 in par_4_set){
    for (par_5 in par_5_set){
      for(par_6 in par_6_set){
        ...
        result_pack[toString(par_4), toString(par_5), toString(par_6)] = best_solution/num_of_tests
      }
    }
  }
  stop_time <- proc.time() - start_time
  
  best_par_6_fitness = min(result_pack[,,1])
  best_par_6_value = par_6_set[1]
  for (par_6 in par_6_set){
    if (min(result_pack[,,toString(par_6)]) < best_par_6_fitness){
      best_par_6_value = par_6   
    }
  }
  par_4_par_5_indexes <- getIndexesOfOptim(result_pack[,,toString(best_par_6_value)], maxSearch = FALSE)
