...
%\end{itemize}
\par
W trakcie badań porównawczych algorytmów wykorzystano i porównano efektywność wykorzystania dwóch \textbf{operatorów mutacji}. Pierwszy z nich (nazywany dalej \emph{mutDefault}) jest domyślnym algorytmem realizującym tę operację według implementacji w ramach pakietu \emph{GA}~\cite{gaPackage}. Drugim wykorzystanym operatorem mutacji (nazywany dalej \emph{mutGauss}) oparty został na podstawowej definicji tego operatora przedstawionej w popularnej pozycji literaturowej \emph{Wykłady z algorytmów ewolucyjnych} Jarosława Arabasa \cite{arabas2001wyklady}. 

\par
Operator \emph{mutDefault} polega na zmianie losowo wybranego atrybutu osobnika na dowolną wartość zawierającą się w zdefiniowanej dla problemu dziedzinie wymiarów. 
